...
Then either the child's own one-line pointer signal (DONE, CONSULTED, BLOCKED,
or GATE-STOPPED with a file/report pointer) when it ended that way, or -- if
it did not -- ordinary metadata and an edited-files COUNT only. Task,
recent-message, last-response, and file bodies are omitted from the fallback;
call ${getSessionResultTool} for the actual list and complete result data.
