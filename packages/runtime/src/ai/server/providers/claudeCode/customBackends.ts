...
/**
 * Catalog routes that can open a lead Claude Agent session.
 *
 * This is the picker's source of truth, and deliberately NOT
 * `CLAUDE_CODE_BACKENDS`: that projection only ever accepted local-proxy
 * routes, so direct-API brains were launchable by persisted id yet invisible
 * in the model list and absent from the enabled-models allow-list. Both gates
 * must agree, so both read this.
 */
export function listLaunchableCatalogRoutes(
  resolution: ProviderCatalogResolution = PROVIDER_CATALOG_RESOLUTION
  ...
  return Object.freeze(
    resolution.entries.filter(
      (entry) =>
        !blockedIds.has(entry.id) &&
        !resolution.disabledIds.includes(entry.id) &&
        entry.capabilities.mainSession &&
        entry.interfaces.some((candidate) =>
          candidate.consumers.includes("claude-agent-main")
        )
    )
  );
