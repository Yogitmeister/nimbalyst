...
describe('main-process lifecycle wiring', () => {
  it('retains ownership across lock acquisition, readiness, and before-quit cleanup', () => {
    ...
    expect(indexSource).toMatch(
      /runIfSingleInstanceLifecycleOwner\(singleInstanceLifecycle, \(\) => \{\s+app\.on\('open-url'/,
    );
    expect(indexSource).toMatch(
      /runIfSingleInstanceLifecycleOwner\(singleInstanceLifecycle, \(\) => \{\s+app\.on\('open-file'/,
    );
    expect(indexSource).toMatch(
      /runIfSingleInstanceLifecycleOwner\(singleInstanceLifecycle, \(\) => \{\s+app\.on\('window-all-closed'/,
    );
  });
});
...
describe('losing-secondary relay and exit path (no Electron process)', () => {
  ...
  it('rejects a relative OS open-file value without relaying and exits exactly once', () => {
    const harness = createHarness(['Nimbalyst.exe']);

    harness.emitOpenFile('relative/path.md');
    harness.expireTimeout();
