...
export interface MobileQueuedPromptIngestDeps {
  ...
  /** Keep queue persistence intact while model recovery owns the dispatch hold. */
...
export async function ingestMobileQueuedPrompts(
  ...
  try {
    ...
    // A recovered model may need to resume before another prompt can run. The
    // durable row and its acknowledgement are already visible to the phone;
    // only the dispatch side effects wait for the existing recovery owner.
    if (!(await deps.canDispatch(sessionId))) {
      deps.logInfo(`[AIService] Mobile queued prompt dispatch deferred for recovery: ${sessionId}`);
      return newPromptsCount;
    }
