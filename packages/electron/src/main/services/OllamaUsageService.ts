 ...
 *   Authorization: <configured Ollama API key> (raw key, NOT "Bearer <key>")
 *
 *   {
 *     "activity": {
 *       "cost": "0.00000",
 ...
 *
 * Unlike the local LiteLLM brain-swap proxy, this direct, read-only GET uses
 * an Ollama API key explicitly configured in Nimbalyst's ai-settings store.
 * It never reads a key from the process or captured shell environment.
 *
...
// real Ollama account credential.
