...
vi.mock("@nimbalyst/runtime/ai/server/SessionStateManager", () => ({
  getSessionStateManager: () => ({
    ...
    // reason: 'session-busy' (including the ordinary "still pending after a
    // declined dispatch" case below, not just an actually-busy session). No
    // test here depends on that wake ever firing, so no-op stubs are enough
    // to keep AIService's real onSessionIdle wiring from throwing.
    on: vi.fn(),
...
describe("SessionTranscript mounted production model-recovery seam", () => {
  ...
  it("consumes captured main registrations through the real store, SDK dispatcher, CLI core, and PTY sink", async () => {
    ...
    // session into a single dispatch": exactly one dispatch, never a second
    // attempt racing to claim the same row) -- so exactly one claim call is
    // the correct outcome here, not a race that happens to resolve to one.
    expect(
